...
class ENVIRONMENT:
    ...
    def send_to(self, sim: pyrosim.pyrosim.Simulator):
        ...
        if self.ID != 0:
            ...
            if self.ID != 1:
                objects[1] = sim.send_box(x=self.x2, y=self.y2 - 5, z=self.z2, length=self.l2, width=self.w2, height=self.h2, collision_group = 'env')
                objects[2] = sim.send_box(x=self.x3, y=self.y3 - 5, z=self.z3, length=self.l3, width=self.w3, height=self.h3, collision_group = 'env')
                objects[3] = sim.send_box(x=self.x4, y=self.y4 - 5, z=self.z4, length=self.l4, width=self.w4, height=self.h4, collision_group = 'env')
